...
public struct LearningEvent: Equatable, Sendable {
    ...
    public init(word: String, date: Date, cleared: Bool) {
        self.word = word
        self.date = date
...
public struct LearningReport: Equatable, Sendable {
    ...
    /// 正答率（クリア / 総イベント、0...1）。イベント0なら0。
    public let accuracy: Double

    public init(totalEvents: Int, distinctWords: Int, learnedWords: Int, activeDays: Int, currentStreakDays: Int, accuracy: Double) {
    ...
    public static let empty = LearningReport(totalEvents: 0, distinctWords: 0, learnedWords: 0, activeDays: 0, currentStreakDays: 0, accuracy: 0)
}
...
public enum LearningReportBuilder {
    ...
    public static func build(events: [LearningEvent], from: Date, to: Date, calendar: Calendar) -> LearningReport {
        ...
        let clearedCount = inRange.reduce(into: 0) { $0 += $1.cleared ? 1 : 0 }
        ...
        return LearningReport(
            ...
            accuracy: total > 0 ? Double(clearedCount) / Double(total) : 0
        )
    }
}
