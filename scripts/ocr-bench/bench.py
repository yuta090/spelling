...
測る指標:
  ...
  - 丁寧さ neatness  : 字の整いを1〜4で返せるか。分布が飽和しないか＋人手ラベルとの一致（丁寧さ報酬の可否判定）
...
SYSTEM_PROMPT = (
    ...
    "5. neatness は『字の丁寧さ・整い具合』を1〜4で評価する。"
    "綴りが合っているか/正解かは一切関係なく、線の整い・形の安定・読みやすさだけで判断する"
    "（間違った綴りでも丁寧な字なら高く、正しい綴りでも雑な字なら低くする）。\n"
    "   1=読みにくい（線が乱れ形が崩れている） 2=ふつうに読める "
    "3=きれいで整っている 4=お手本のように整っている。\n"
    "次のJSONだけを返す（前後に文章を付けない）:\n"
    '{{"predicted_word": "", "legible": true, "matches_target": true, "neatness": 2, "comment": ""}}'
)
...
def load_samples_supabase():
    ...
    r = requests.get(
        ...
        params={
            "select": "id,storage_path,target,ground_truth,legible,neatness",
    ...
    for row in rows:
        ...
        samples.append({
            ...
            "neatness_truth": clamp_neatness(row.get("neatness")),
            "image": obj.content,
        })
    return samples
...
def call_model(model: str, sample: dict) -> dict:
    ...
    return {
        ...
        "neatness": clamp_neatness(parsed.get("neatness")),
...
def clamp_neatness(v) -> "int | str":
    """モデルが返した neatness を 1〜4 の int に正規化。範囲外/欠損/非数値/真偽値は ""（無効）。"""
    if isinstance(v, bool):  # bool は int のサブクラス。True→1 と誤って拾わないよう先に弾く
        return ""
    try:
        n = int(round(float(v)))
    except (TypeError, ValueError):
        return ""
    return n if 1 <= n <= 4 else ""
...
def main():
    ...
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow([
            ...
            "ground_truth", "legible_truth", "neatness_truth", "verdict_correct", "recognized_text",
            "predicted_word", "legible", "matches_target", "neatness", "comment",
            ...
            for model in MODELS:
                try:
                    ...
                    res = {"latency_ms": "", "predicted_word": "", "legible": "",
                           "matches_target": "", "neatness": "", "comment": "",
                           "prompt_tokens": "", "completion_tokens": ""}
                    err = str(e)[:200]
                w.writerow([
                    model, s["id"], s["target"], s.get("label_kind", ""),
                    s.get("ground_truth", ""), s.get("legible_truth", ""), s.get("neatness_truth", ""),
                    s.get("verdict_correct", ""), s.get("recognized_text", ""),
                    res["predicted_word"], res["legible"], res["matches_target"], res["neatness"], res["comment"],
...
def summarize(rows):
    ...
    # 丁寧さ(neatness)が4段に「使えるか」の一次判定。
    #   - 分布が一値に張り付く(飽和)なら Vision confidence と同じく無価値 → 段数を減らす根拠。
    #   - neatness_truth(人手1〜4)がある行では一致率(完全/±1)を出す。±1は主観評価の実用ライン。
    print("\n=== 丁寧さ neatness 1〜4（飽和チェック＋人手一致） ===")
    print(f"{'model':32s} {'n':>3} {'1':>3} {'2':>3} {'3':>3} {'4':>3} {'無効':>4} {'対':>4} {'一致':>6} {'±1':>6}")
    # bool は int のサブクラスなので type() で厳密判定（True/False を段階値として拾わない）。
    def is_tier(v):
        return type(v) is int and 1 <= v <= 4
    for model, rs in by_model.items():
        ok = [r for r in rs if not r["err"]]
        vals = [r["res"]["neatness"] for r in ok]
        dist = {t: sum(1 for v in vals if is_tier(v) and v == t) for t in (1, 2, 3, 4)}
        invalid = sum(1 for v in vals if not is_tier(v))
        # 人手ラベルとの一致（モデル/人手の両方が有効な 1〜4 の行だけ）
        paired = [(r["sample"].get("neatness_truth"), r["res"]["neatness"]) for r in ok
                  if is_tier(r["sample"].get("neatness_truth")) and is_tier(r["res"]["neatness"])]
        if paired:
            exact = sum(1 for t, p in paired if t == p) / len(paired)
            within1 = sum(1 for t, p in paired if abs(t - p) <= 1) / len(paired)
            agree_str, within_str = f"{exact:.0%}", f"{within1:.0%}"
        else:
            agree_str = within_str = "-"
        print(f"{model:32s} {len(vals):>3} {dist[1]:>3} {dist[2]:>3} {dist[3]:>3} {dist[4]:>3} "
              f"{invalid:>4} {len(paired):>4} {agree_str:>6} {within_str:>6}")
    print("  ※ 分布が1段に偏る/無効が多いモデルは丁寧さ判定に不適。'対'=人手ラベルと突き合わせできたペア数（neatness_truth を埋めた時のみ>0）。")
