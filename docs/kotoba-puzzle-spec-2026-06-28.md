...
Status: **実装中**（並べ替え・穴埋め選択・混合・単語リスニングは動作／リスニング穴埋め以降は計画）。
...
3. confusables_sound のビルド/検証スクリプト（wordbank存在・band・重複チェック→同梱JSON）→ `WordListeningView` のインラインCSVを同梱データに差し替え。
