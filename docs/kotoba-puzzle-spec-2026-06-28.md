...
| 文バンク `sentence_bank`（同梱） | `iPadPrototype/Resources/sentence_bank.json`（33文・学年/トークン/文法タグ済み） | ✅ 生成・同梱（`SentenceBankBundle` が読込） | curated を学年タグ付けして引く。Tanaka 自動抽出は次サブ手 |
| 文バンク 原本（curated 素） | `scripts/person_templates.authoring.json` の `fallback*`（承認済み） | ✅ ビルドツールの入力 | 前処理 `sentence-bank-build` の素 |
| `wordbank.sqlite` | `iPadPrototype/Resources/`（読取専用） | ✅ | gloss/examples/level（NGSLバンド・Dolch） |

文バンク前処理の運用: `swift run sentence-bank-build [--write]`（curated 素＋wordbank の `level`→検証→同梱JSON）。**壁は「文中の見える内容語すべて」に効く**（著者 lemma に頼らず、トークンから内容語を導き band 解決）。`level` 未収録語は **②と同じく警告に留め**、curated は著者宣言バンド(`gradeBand`)を信頼して採用（gloss/level 欠落で良データを壊さない）。語数・文法上限・不適切語ブロック・重複は不採用。id は文から決定論 UUIDv5（再実行で同一＝差分安定）。
...
| `SimpleLemmatizer.swift` | 簡易見出し語化（-s/-es/-ies/-ed/-ing/-er/-est＋不規則小辞書）。文バンクの学年解決用 |
| `SentenceBankBuilder.swift` | 文バンク前処理（トークン化→内容語を level 解決→学年/文法/適切性の壁→決定論id→JSON serialize/decode）＋ CLI `Sources/SentenceBankBuild` |
...
| `SentenceBankBundle.swift` | 同梱 `sentence_bank.json` を読み `SentenceItem` 群へ（出題プール・一度だけキャッシュ） |
...
4. ✅ 文バンク前処理（curated 学年タグ付け）→ 同梱 `sentence_bank.json`（33文）生成。Core `SimpleLemmatizer`/`SentenceBankBuilder`（TDD）＋ CLI `sentence-bank-build` ＋ アプリ `SentenceBankBundle`。**壁は見える全内容語に効く**。残り＝(a) Tanaka 自動抽出（量を足す・grammar=nil 可）、(b) 出題画面を `SentenceBankBundle` 配線（現状は person_templates 直結のデモ）。
