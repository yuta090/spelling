...
Status: **実装中**（並べ替え・穴埋め選択・混合は動作／リスニングは土台実装中）。
...
| **単語リスニング** | 音を聞いて正しい綴りを選ぶ | tap | 決定的 | 🟡 Core実装済(本リスニングブランチ)。UI未 |
| **リスニング穴埋め** | 文の空所・**設問中は無音→回答後に音**・公共用にON/OFF | tap | 決定的 | 🟡 設計済。Core は穴埋め＋音声で再利用予定。UI未 |
...
| リスニングおとり辞書 | `scripts/confusables_sound_draft.csv`（40行・`approved=1`） | ✅ main にあり | 単語/リスニング穴埋めのおとり |
...
おとり辞書の運用: AI生成は必ず `approved=0`、人が音の近さを承認した行を `approved=1`。同梱は `approved=1` のみ（ビルド/検証スクリプト §confusables-sound §5 は未実装）。
...
## 7. 次の実装ステップ
1. 🟡 単語リスニング：Core（済）→ **音声つきプレイ画面＋リスニングON/OFFトグル**（UI）。
2. リスニング穴埋め：`ClozeChoiceGenerator` に confusables おとりを供給＋回答後再生のUI。
3. confusables_sound のビルド/検証スクリプト（wordbank存在・band・重複チェック→同梱JSON）。
