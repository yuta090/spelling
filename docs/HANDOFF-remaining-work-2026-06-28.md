...
2. **取り込み Edge Function**（service_role）:
   - `appstore-notify`: ASSN V2 を受信 → **JWS署名検証(Apple x5c証明書チェーン)** → decode → `app.upsert_entitlement` 呼び出し（更新/解約/返金/猶予を追従）。
   - 任意 `entitlements-sync`: 購入直後に iPad が StoreKit2 の signedTransaction を渡し、Edge が App Store Server API で検証 → 同 upsert（即時反映）。
